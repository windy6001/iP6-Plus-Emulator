...
RINGBUFFER *ringbuffer_Open(int size) 
{
	...
	buff->buffer =
	    (short *)malloc(buff->in_length * sizeof(short)); /* RING BUFFER のバッファを作成 */
	if (buff->buffer != NULL) {
...
int ringbuffer_Read(RINGBUFFER *buff, short *buffer, int size) 
{
	...
	if (buff == NULL) {
		printf("not initialize ring buffer");
		return 0;
	}
	if (buff->buffer == NULL) {
		printf("not initialize ring buffer");
...
int sense_keybuffer(KEYBUFFER *_keybuffer, char *chr, int *keydown, int *scancode,
                    int *osdkeycode) 
{
	int ret = 0;
...
void close_keybuffer(KEYBUFFER *_keybuffer) { free(_keybuffer); }
