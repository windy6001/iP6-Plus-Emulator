...
そのため、２と３を用意しました。バンク切り替え関係なく、RAMだけが見れます。
...
　　　SR MODE6 SCREEN 2 のVRAMを拡張メモリーに保存したり、ロードしたりします。
...
LLIST/LPRINT　命令で、ホストOSのテキストファイル(printer.txt)に出力します。
初回起動時は、全ての文字を　半角で出力します。ひらがなは、カタカナで出力されます。

設定ファイルの、PrnMode=0 を、PrnMode=1 に変更して、再起動すると、かな文字などは、全角で出力されます。なおかつ、ひらがなは、ひらがな　で出力されるようになります。
とりあえず、英数字と、かな文字のみの対応になります。
...
　- プリンターに出力すると、ホストOSのテキストファイル(printer.txt)に出力する
　- ジョイスティックの上下左右を、十字ボタンで行うようにした
...
　- アナログシンセサイザー(AMDEKのCMU-800)で、４オクターブ目のFの音が出てなかったのを修正
　- ４つめのチャンネル以降の演奏指示を無視するように修正(PSG音源の制約のため)
　- テンポをなるべく合わせた。
